...
/**
 * @author rusu_ Diese ist die Board Klasse. Da wird den Board als ein Matrix
 *         gemacht.
 */
class Board implements ObservableBoard {
	...
	MoveOutcome move(MoveDescr md) {
		byte c = md.getCol();
		if (moveState == Position.whiteWon || moveState == Position.blackWon || topincol[c] == 6) {
			return MoveOutcome.illegal;
		} else {
			topincol[c]++;
			byte colour = (moveState == Position.whiteNext) ? Position.white : Position.black;
	...
	private boolean checkwin(byte colour) {
		for (int ic = 1; ic <= 4; ic++) {
			for (int ir = 1; ir <= 3; ir++) {
				if ((get(ic, ir) == colour && get(ic + 1, ir) == colour && get(ic + 2, ir) == colour
						&& get(ic + 3, ir) == colour)
						|| (get(ic, ir) == colour && get(ic, ir + 1) == colour && get(ic, ir + 2) == colour
								&& get(ic, ir + 3) == colour)
						|| (get(ic, ir) == colour && get(ic + 1, ir + 1) == colour && get(ic + 2, ir + 2) == colour
		...
		for (int ic = 5; ic <= 7; ic++) {
			for (int ir = 1; ir <= 3; ir++) {
				if ((get(ic, ir) == colour && get(ic, ir + 1) == colour && get(ic, ir + 2) == colour
						&& get(ic, ir + 3) == colour))
					return true;
			}
		}
